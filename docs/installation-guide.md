...
### 5. LM Studio (Bắt buộc cho AI Semantic Search v2)
LM Studio đóng vai trò làm máy chủ suy luận AI cục bộ (Local Inference Server), giúp sinh vector nhúng (embedding) 768 chiều cực nhanh bằng GPU/CPU mà không cần tốn chi phí đám mây.
- **Tải về:** [LM Studio](https://lmstudio.ai/)
- **Yêu cầu thiết lập:** Tải mô hình `nomic-embed-text-v1.5` và bật Local Server tại cổng `1234`.
...
### Bước 6: Khởi chạy Máy chủ AI và Nạp dữ liệu Vector (Bắt buộc cho Semantic Search)
Tính năng tìm kiếm ngữ nghĩa v2 yêu cầu máy chủ AI cục bộ hoạt động để sinh và so khớp vector:
1. Mở ứng dụng **LM Studio**, chọn nạp mô hình `nomic-embed-text-v1.5` đã tải và bấm **Start Server** ở cổng `1234`.
2. Mở một Tab Terminal mới, di chuyển đến thư mục `frontend/` và chạy lệnh nạp vector nền:
   ```powershell
   cd frontend
   node scripts/seed-vectors.js
   ```
   *Script này sẽ quét qua toàn bộ 200+ văn bản mẫu, sinh vector 768 chiều từ LM Studio và lưu thẳng vào Supabase. Đảm bảo LM Studio vẫn chạy trong lúc sử dụng chức năng AI Search trên UI.*
